	...
	function kapee_minicart_slide(){ ?>
		...
		<div class="kapee-minicart-slide">
			<div class="minicart-header">
				<h3 class="minicart-title"><?php echo apply_filters( 'kapee_mini_cart_header_text', esc_html__('Mi Carrito','kapee' ) );?></h3>
				<a href="#" class="close-sidebar"><?php esc_html_e( 'Cerrar', 'kapee' ); ?></a>
			</div>
...
if( ! function_exists( 'kapee_empty_mini_cart_button' ) ) :
	...
	<p class="woocommerce-empty-mini-cart__buttons">
		<a class="button" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>"><?php echo apply_filters( 'kapee_empty_mini_cart_button_text', esc_html__( 'Comprar', 'kapee' ) );?></a>
	</p>
	<?php }
endif;
...
if ( ! function_exists( 'kapee_product_loop_view' ) ) :
	...
	function kapee_product_loop_view() {
		...
		<div class="products-view">
			<a class="grid-view <?php echo esc_attr( $product_view =='grid-view' ) ? 'active' : ''; ?>" data-shopview="grid-view" href="<?php echo esc_url( add_query_arg( array( 'view' => 'grid-view' ) ) );?>"><?php esc_html_e('Vista Grid','kapee');?></a>
			<a class="list-view <?php echo esc_attr( $product_view =='list-view' ) ? 'active' : ''; ?>" data-shopview="list-view" href="<?php echo esc_url( add_query_arg( array( 'view' => 'list-view' ) ) );?>"><?php esc_html_e('Vista en Lista','kapee');?></a>
		</div>
		<?php 
	}
endif;
	...
	function kapee_product_loop_show() {
		...
		if( !empty( $show_numbers ) ) { ?>
			<div class="product-show">
				<span><?php esc_html_e('Mostrar:','kapee');?></span>
...
if ( ! function_exists( 'kapee_product_filter_top' ) ) :
	...
	function kapee_product_filter_top() {
		...
		?>
		<span class="kapee-product-filter-btn"><?php echo esc_html__('Filtrar','kapee');?></span>
...
if ( ! function_exists( 'kapee_product_loop_compare_button' ) ) :
	...
	function kapee_product_loop_compare_button() {
		...
		global $product;
		$id = $product->get_id();
		$button_text = get_option( 'yith_woocompare_button_text', esc_html__( 'Comparar', 'kapee' ) );
			...
			<?php if ( ! empty( $prev ) ): ?>
				<div class="product-nav-btn product-prev">
					<a href="<?php echo esc_url( $prev->get_permalink() ); ?>">
						<?php esc_html_e('Producto Previo', 'kapee'); ?>
					</a>				
			...
			<?php if ( ! empty( $next ) ): ?>
				<div class="product-nav-btn product-next">				
					<a href="<?php echo esc_url( $next->get_permalink() ); ?>">
						<?php esc_html_e('Siguiente Producto', 'kapee'); ?>
					</a>
					<div class="product-info-wrap kapee-arrow">
						<div class="product-info">
							<div class="product-thumb">
...
if ( ! function_exists( 'kapee_woocommerce_before_account_orders' ) ) :
	...
	function kapee_woocommerce_before_account_orders( $has_orders) {
		...
		<div class="section-title">
			<h2><?php esc_html_e( 'Mis Pedidos', 'kapee' ); ?></h2>
			<p><?php esc_html_e( 'Sus pedidos recientes se muestran en la siguietne tabla.', 'kapee' ); ?></p>
		</div>
		<?php
	}
endif;
...
if ( ! function_exists( 'kapee_woocommerce_before_account_downloads' ) ) :
	...
	function kapee_woocommerce_before_account_downloads( $has_orders) {
		?>
		<div class="section-title">
			<h2><?php esc_html_e( 'Descargas', 'kapee' ); ?></h2>
			<p><?php esc_html_e( 'Tus descargas de archivos de producto como manuales o afines se encuentran en esta tabla.', 'kapee' ); ?></p>
		</div>
		<?php
	}
endif;
...
if ( ! function_exists( 'kapee_woocommerce_my_account_my_address_description' ) ):
	...
	function kapee_woocommerce_my_account_my_address_description( $address_desc ) {
		...
		$address_title .= '<h2>'.esc_html__('Dirección','kapee').'</h2>';
...
if ( ! function_exists( 'kapee_woocommerce_myaccount_edit_account_heading' ) ) :
	...
	function kapee_woocommerce_myaccount_edit_account_heading() {
		?>
		<div class="section-title">
			<h2><?php esc_html_e( 'Mi cuenta', 'kapee' ) ?></h2>
			<p><?php esc_html_e( 'Edita los detalles de tu cuenta, o cambia la contraseña.', 'kapee' ); ?></p>
		</div>
		<?php
	}
endif;