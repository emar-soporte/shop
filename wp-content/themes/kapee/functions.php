 ...
 * @since 1.0.0
 */
update_option( 'kapee_is_activated', '1' );
update_option( 'envato_purchase_code_24187521', 'purchase_code' );
/*-----------------------------------------------------------------------*/
/* Define Constants.
/*-----------------------------------------------------------------------*/
...
function translate_text($translated) {
     $translated = str_ireplace('Search Results',  'Resultados de Búsqueda',  $translated);
     return $translated;
}
...
function custom_paypal_button_text( $translated_text, $text, $domain ) {
   switch ( $translated_text ) {
      case 'Ir a PayPal' :
         $translated_text = __( 'Pagar', 'woocommerce' );
         break;
   }
