 ...
 */
class vcDokanVendors extends WPBakeryShortCode {

    function __construct() {
        $this->_mapping();
        add_shortcode( 'kapee_dokan_vendors', array( $this, '_html' ) );
	}
	public function _mapping() {
		if ( !defined( 'WPB_VC_VERSION' ) ) { return; }
		
		vc_map( array(
			'name' 			=> esc_html__( 'Dokan Vendors', 'kapee-extensions' ),
			'base' 			=> 'kapee_dokan_vendors',
			...
			'params' 		=> array(
				...
				array(
					...
					'std' 			=> 'meta_value',
					'value'			=> array(
						esc_html__('Store Name','kapee-extensions') 	=> 'meta_value',
	...
	public function _html( $atts, $content ) {
		...
		extract( $args );
		$args['id']				= kapee_uniqid('kapee-dokan-vendors-');
		$class					= array( 'kapee-element', 'kapee-dokan-vendors', 'kapee-vendors-'.$style,  $el_class );
		...
		ob_start();
			kapee_get_pl_templates( 'shortcodes/dokan-vendors/'.$style, $args );			
		return ob_get_clean();
	}	
}
new vcDokanVendors();