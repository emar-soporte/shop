...
if( ! function_exists( 'kapee_title_type' ) && function_exists( 'vc_add_shortcode_param' ) ) {
	function kapee_title_type( $settings, $value ) {		
		...
		return $u . '<h4' . $c . '>' . $settings['content'] . '</h4>' . ( $u ? '</a>' : '' ) . '<input type="hidden" name="' . $settings['param_name'] . '" class="wpb_vc_param_value ' . $settings['param_name'] . ' '.$settings['type'].'_field" value="'.$value.'" />';
	}
